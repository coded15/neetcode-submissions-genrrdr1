   ...
   public:
    ...
    bool dfs(const string& s, const vector<string>& wordSet, int i) {
        ...
        for (const string& word : wordSet) {
            ...
            // don't forget to have an equality with the length of string s, while comparing it with i + word.length(), as we have set memo[s.length()] as true
            // dont use "i + word.length()", in  (s.substr(i, i + word.length()) == word) just use word.length
